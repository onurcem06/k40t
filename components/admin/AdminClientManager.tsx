...
const AdminClientManager: React.FC<AdminClientManagerProps> = ({ clients, onUpdateClients }) => {
  ...
  const handleFile = async (file: File, callback: (base64: string) => void) => {
    if (!file.type.startsWith('image/')) return;
    try {
      // Logolar için PNG formatını ve şeffaflığı koruyoruz
      const compressed = await compressImage(file, 500, 0.8, 'image/png');
      callback(compressed);
    } catch (err) {
      console.error(err);
      alert("Logo işlenemedi.");
    }
  };
