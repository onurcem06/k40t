      ...
      {showAddForm && (
        <form onSubmit={handleAddUser} className="bg-[#0D1225] p-10 rounded-[3rem] border border-orange-500/20 grid grid-cols-1 md:grid-cols-4 gap-6 animate-in zoom-in-95">
          ...
          <div className="space-y-2">
            ...
            <input required minLength={6} type="password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} className="w-full bg-black/40 border border-white/10 p-4 rounded-xl text-white font-bold" placeholder="Min. 6 karakter" />
          </div>
          <div className="space-y-2">
            ...
            <div className="lg:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div
                  ...
                  className={`p-6 rounded-3xl border transition-all cursor-pointer flex items-center gap-6 ${(u.permissions as any)[p.id] ? 'bg-orange-600/10 border-orange-500/30' : 'bg-black/10 border-white/5 opacity-50'
                    }`}
                >
